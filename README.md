...
* *Parsers* - Interface Layer - contains code to translate from input file (via reasonably sophisticated parsing)  and to make appropriate calls into the Application layer (via the 'service API' - which could easily have been wrapped by REST - for remote client access - however this is not shown).
...
The <u>products</u> that the company produces are currently:  Chocolate Cake, Iced Buns, Victoria Sponge and Malt Loaf, however the application should be user configurable (via file inputs) to enable the <u>generic addition of these and other products and their related ingredients</u>. Generic means including products as yet undefined.
...
* The ability to <u>easily change the ‘mix’ of raw materials</u> for each product, to see what impact that would have on stock days. File editing is acceptable for this.
...
* <u>Unit types</u> should be defined in a <u>generic and easily extensible manner</u>. The application should be set up to read by <u>kg</u> (kilogram) and <u>g</u> (gram), and also <u>litres</u> and <u>ml</u> (millilitres). The application should deal  with <u>conversion of units</u> as appropriate.
...
* The <u>names of all input files</u> should be provided <u>on the *command line*</u> in the following order when running the program: ingredients stock level file; product ingredients list file; product sales forecast file, e.g. 
  ...
  See "Appendix" for exact details of the file formats.  The '.eg' extension is optional.
...
* A <u>ProductLine</u> has many Products (chocolate cake, malt loaf ...), Ingredients (sugar, vanilla essence ...) and <u>Unit</u>s (kgs, litres ...) associated with it.
* Each <u>Product</u> on the <u>ProductLine</u> may have an associated <u>Forecast</u> which says how many items are predicted to be sold on a particular date. Forecast  is an association entity, in that it is associated with a particular ProductLine and a particular Product.
* A <u>Product</u> has many <u>Ingredient</u>s, each individual Ingredient has a <u>quantity</u> associated with it (e.g. malt loaf has 3g of sugar in it). Quantity, in the model shown above, is an association attribute, being linked to a particular <u>Product</u> and a particular <u>Ingredient</u>.
* Each <u>Ingredient</u> has a <u>Unit</u> associated with it (e.g. vanilla essence is measured in litres).

Other notes:

* Most entities (classes) have a name, the exceptions being quantity which is really just a number and an attribute of the <u>Product</u>/<u>Ingredient</u> association, and <u>ProductLine</u> - which is really there to bind everything together. Note that the model above could support multiple <u>ProductLine</u>s.

### Package model

The code implementation stored here has the following package structure and dependencies. Not all dependencies are shown, as dependencies are transitive (so Parsers know about Products, etc...);  Utils is misc useful stuff and may be depended upon by all higher level (in terms of layers) packages; Dependencies are <u>unidirectional</u> and <u>acyclic</u> - as shown by arrowheads (see the ADP - Acyclic Dependency Principle).
...
* <u>ProductLine</u> provides a “<u>Service Oriented Architecture</u>” access point to the domain elements. Control and Parser(s) use this. In the event of a <u>graphical</u> UI being required, ProductLine would still provide the major point of access into the core of the system. Many of the required methods are already there - as is often the case in applications that have multiple Interfaces (e.g. A REST API and a local file format).
...
* <u>Products</u> keep a list of <u>Ingredient</u>s that they use. Again, HashMaps and String names.
* <u>Unit</u>s are part of the <u>Ingredients</u> package. <u>Ingredient</u>s keep tabs on their Units (e.g. sugar is in grams) and this is stored within the Ingredient. Unit is an *enum* and also provides some unit conversion aids (e.g. converting litres to ml). This be extended to cope with “tbsp”, etc. with relative ease (but isn't - contributions welcome).
* <u>Parser</u>s raise <u>ParseError</u> exceptions when something is wrong. <u>Control</u> deals with the presentation of this information to the user. None of the lower level packages (<u>Product</u>, <u>Ingredient</u> …) know anything about this (as per the ARM's separation of concerns - aka keep interface and business logic separate).
* To facilitate unit tests, the <u>Parser</u>s take arrays of Strings (instead of files). This also has the beneficial effect that comment lines (beginning with ‘#’) can be stripped out before the Parser gets to see them, making the syntax checking cleaner.
...
* The top level directory (a.k.a in this description: $ROOT) contains all the packages, and three (Unix Shell) scripts:
  ...
  * runTests.sh - runs all the unit tests (all packages contain these, except the Control package).
  ...
  Take a look inside these shell scripts to vary the files used or the tests, etc. They are very simple (Linux based).
...
- To generate syntax errors (or semantic errors)  just edit the test data files (all of which have a *.eg* extension) and you'll see the errors being reported when you run the application or the related test scripts.
...
* the lines below a product name list - in comma delimted form - amount, unit, stock-item.
* a blank line indicates the end of a product - the next line may contain the start of another product description.
...
For the demand forecast, the first line contains a list of dates: 

```
‘,[date1],[date2],… ,[dateN]’, ... etc
```

the lines below this contain the expected demand for each finished product for each date given: 

```‘[finished product name],[date1 demand],[date2 demand],….,[datey demand]’, e.g.:```
...
* CLASSPATH is set in: classpath.shi - a shared bash script file - which is 'included' (using '.') in other bash scripts (.sh).
